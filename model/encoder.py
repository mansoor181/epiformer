...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, key_value, attention_mask=None):
        ...
        if attention_mask is not None:
            # print("attention masks provided..")
...
class EpiformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # # Separate ResMP layers for antigen and antibody
        ...
        if use_layer_norm:
            ...
            # self.ln_ag_post = nn.LayerNorm(residue_dim)
            # self.ln_ab_post = nn.LayerNorm(residue_dim)
    ...
    def forward(self, ag_hetero_data, ab_hetero_data, ag_features, ab_features):
        ...
        # ag_alpha = torch.tensor(0.1)  # Small initial weight for stability
        # ab_alpha = torch.tensor(0.1)
        
        # ============ Lean Residual Gating ============
        # Residual connection with learnable scalar gates: h ← h + α·Attn(h)
        # ag_out = ag_input + ag_resmp_out + self.ag_alpha * ag_attn_out  
        # ab_out = ab_input + ab_resmp_out + self.ab_alpha * ab_attn_out

        ag_out = ag_input + ag_resmp_out + self.ag_alpha * ag_final  
        ab_out = ab_input + ab_resmp_out + self.ab_alpha * ab_final

        """
        TODO: remove the input residual connections
        """
        # ag_out =  ag_resmp_out + self.ag_alpha * ag_attn_out  
        # ab_out =  ab_resmp_out + self.ab_alpha * ab_attn_out
        
        # # Post-normalization
        # if self.use_layer_norm:
        #     ag_out = self.ln_ag_post(ag_out)
        #     ab_out = self.ln_ab_post(ab_out)
...
"""
NOTE:
The gates will typically show:
    - Higher weights for geometric features near binding sites
    - Increased PLM weights for conserved residues
    - Adaptive atom/edge weights depending on local environment
"""
...
class EpiformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_plm:
            # geometric(105) + plm(128) = 233 -> residue_dim(105)
            ...
            # geometric(105) only -> residue_dim(105)
    ...
    def forward(self, hetero_data, ag_chain_type='ag', ab_chain_type='ab'):
        ...
        # print(ag_combined.shape, ab_combined.shape)
        # print(self.feature_fusion) N_ag x 233 -> 233 x 128
...
class TestEpiformerBlock(nn.Module):
    """Single epiformer block: ResMP followed by Cross-Attention"""
    ...
    def __init__(self, 
        ...
        self.ag_resmp = EGNN_ResMP(
            ...
            edge_dim=edge_dim,
            hidden_dim=residue_hidden_dim,
            num_layers=2,  # Single layer per block
            num_relations=num_relations,
            dropout=dropout,
            layer_norm=use_layer_norm
        )
        ...
        # Pair representation (optional)
        self.use_pair_repr = use_pair_repr
        if use_pair_repr:
            self.pair_init = PairRepresentation(residue_dim, pair_dim, dropout)
            self.triangle_update = TriangleUpdate(pair_dim, dropout)
        ...
        # Layer normalization for all components (pre-norm everywhere for better gradient flow)
        self.use_layer_norm = use_layer_norm
        if use_layer_norm:
            # Pre/Post norm for ResMP
            self.ln_ag_resmp_pre = nn.LayerNorm(residue_dim)
            self.ln_ab_resmp_pre = nn.LayerNorm(residue_dim)
            # self.ln_ag_resmp_post = nn.LayerNorm(residue_dim)
            # self.ln_ab_resmp_post = nn.LayerNorm(residue_dim)
            
            # Pre/Post norm for Cross-Attention
            self.ln_ag_attn_pre = nn.LayerNorm(residue_dim)
            self.ln_ab_attn_pre = nn.LayerNorm(residue_dim)
            # self.ln_ag_attn_post = nn.LayerNorm(residue_dim)
            # self.ln_ab_attn_post = nn.LayerNorm(residue_dim)
            
            # Pre/Post norm for Feedforward Networks
            self.ln_ag_ffn_pre = nn.LayerNorm(residue_dim)
            self.ln_ab_ffn_pre = nn.LayerNorm(residue_dim)
            self.ln_ag_ffn_post = nn.LayerNorm(residue_dim)
            self.ln_ab_ffn_post = nn.LayerNorm(residue_dim)
    ...
    def create_batch_attention_mask(self, ag_batch, ab_batch):
        ...
        Args:
            ag_batch: [n_ag_total] - batch indices for each AG residue
            ab_batch: [n_ab_total] - batch indices for each AB residue
            
        Returns:
            mask: [n_ag_total, n_ab_total] - True where attention is allowed (same complex)
        """
        # ag_batch: [0,0,0,1,1,2,2,2,2] (3 complexes with 3,2,4 AG residues)
        # ab_batch: [0,0,1,1,1,2,2,2]   (3 complexes with 2,3,3 AB residues)
        
        # Expand batch indices for comparison
        ag_expanded = ag_batch.unsqueeze(1)  # [n_ag, 1]
        ab_expanded = ab_batch.unsqueeze(0)  # [1, n_ab]
        
        # Create mask: True where batch indices match (same complex)
        mask = (ag_expanded == ab_expanded)  # [n_ag, n_ab]
        
        return mask
        
    def forward(self, ag_hetero_data, ab_hetero_data, ag_features, ab_features, pair_repr=None):
        """
        Forward pass through one epiformer block
        ...
        Returns:
            ...
            ab_out: Updated antibody features [n_ab, d_model]
        """
        # Update hetero_data with current features
        ag_hetero_data['ag_res'].x = ag_features
        ab_hetero_data['ab_res'].x = ab_features
        
        # Extract batch information for attention masking
        ag_batch = ag_hetero_data['ag_res'].batch  # [n_ag_total]
        ab_batch = ab_hetero_data['ab_res'].batch  # [n_ab_total]
        
        # Create batch-aware attention masks
        """
        - construct attention masks to constrain attention to same-complex interactions
            1. Mask Format: Use boolean mask (True = attend, False = mask)
            2. Efficiency: Create mask once per block, reuse for both AG→AB and AB→AG attention
            3. Memory: Mask is sparse (block-diagonal structure), could optimize later
            4. Gradient Flow: Masked positions get -∞ logits → 0 attention weights → no gradients
        """
        ag_to_ab_mask = self.create_batch_attention_mask(ag_batch, ab_batch)  # [n_ag, n_ab]
        ab_to_ag_mask = ag_to_ab_mask.transpose(0, 1)  # [n_ab, n_ag]
        # print(ag_to_ab_mask)
        
        # ============ ResMP Step with Pre/Post Norm ============
        # # Pre-norm for ResMP
        # if self.use_layer_norm:
        ag_resmp_input = self.ln_ag_resmp_pre(ag_features)
        ab_resmp_input = self.ln_ab_resmp_pre(ab_features)
        #     ag_hetero_data['ag_res'].x = ag_resmp_input
        #     ab_hetero_data['ab_res'].x = ab_resmp_input
        # else:
        #     ag_resmp_input = ag_features
        #     ab_resmp_input = ab_features
        
        # ResMP step using separate modules
        ag_resmp_out, _ = self.ag_resmp(ag_hetero_data, 'ag_res')
        ab_resmp_out, _ = self.ab_resmp(ab_hetero_data, 'ab_res')
        
        # ResMP residual connection + dropout (correct: input + output)
        ag_resmp_out = ag_resmp_input + self.dropout(ag_resmp_out)
        ab_resmp_out = ab_resmp_input + self.dropout(ab_resmp_out)
        
        # # Post-norm for ResMP
        # if self.use_layer_norm:
        #     ag_resmp_out = self.ln_ag_resmp_post(ag_resmp_out)
        #     ab_resmp_out = self.ln_ab_resmp_post(ab_resmp_out)
        
        # ============ Cross-Attention Step with Pre/Post Norm ============
        # Pre-norm for cross-attention
        if self.use_layer_norm:
            ag_attn_input = self.ln_ag_attn_pre(ag_resmp_out)
            ab_attn_input = self.ln_ab_attn_pre(ab_resmp_out)
        else:
            ag_attn_input = ag_resmp_out
            ab_attn_input = ab_resmp_out
        
        # Add batch dimension for attention
        ag_attn_batch = ag_attn_input.unsqueeze(0)  # [1, n_ag, d_model]
        ab_attn_batch = ab_attn_input.unsqueeze(0)  # [1, n_ab, d_model]
        
        # Cross-attention: AG attends to AB, AB attends to AG (with batch-aware masks)
        ag_cross_out = self.ag_cross_attn(ag_attn_batch, ab_attn_batch, ag_to_ab_mask).squeeze(0)  # [n_ag, d_model]
        ab_cross_out = self.ab_cross_attn(ab_attn_batch, ag_attn_batch, ab_to_ag_mask).squeeze(0)  # [n_ab, d_model]
        
        # Attention residual connections + dropout (correct: input + output)
        ag_attn_out = ag_attn_input + self.dropout(ag_cross_out)
        ab_attn_out = ab_attn_input + self.dropout(ab_cross_out)
        
        # # Post-norm for cross-attention
        # if self.use_layer_norm:
        #     ag_attn_out = self.ln_ag_attn_post(ag_out)
        #     ab_attn_out = self.ln_ab_attn_post(ab_out)
        # else:
            # ag_attn_out = ag_out
            # ab_attn_out = ab_out
        
        # ============ Feedforward Networks Step ============
        # Pre-norm for feedforward
        if self.use_layer_norm:
            ag_ffn_input = self.ln_ag_ffn_pre(ag_attn_out)
            ab_ffn_input = self.ln_ab_ffn_pre(ab_attn_out)
        else:
            ag_ffn_input = ag_attn_out
            ab_ffn_input = ab_attn_out
        
        # Apply feedforward networks
        ag_ffn_out = self.ag_ffn(ag_ffn_input)
        ab_ffn_out = self.ab_ffn(ab_ffn_input)
        ...
        # # ============ Pair Representation Step ============
        # if self.use_pair_repr:
        #     # Initialize pair representation if not provided
        #     if pair_repr is None:
        #         pair_repr = self.pair_init.init_from_nodes(ag_features, ab_features)
            
        #     # Update pair representation with triangle multiplicative update
        #     pair_repr = self.triangle_update(pair_repr, ag_to_ab_mask)
            
        #     return ag_final, ab_final, pair_repr
        # else:
            
        return ag_final, ab_final

        # return ag_attn_out, ab_attn_out
