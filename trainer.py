...
def get_run_id(cfg):
    """Generate unique run ID. Uses cfg.run_id if provided, otherwise generates from timestamp."""
    if cfg.get("run_id"):
        return cfg.run_id
...
def main(cfg: DictConfig):
    ...
    dataset_path = proj_dir / "data/epiformer" / dataset_filename
    ...
    if use_wandb:
        ...
        if wandb.run:
            ...
            print(f"WANDB_RUN_URL: {wandb.run.url}")
    ...
    # Group sweep runs under a sweep-specific subdirectory
    sweep_id = os.environ.get("WANDB_SWEEP_ID", "")
    if sweep_id:
        checkpoint_dir = proj_dir / Path(cfg.callbacks.model_checkpoint.dirpath) / f"sweep_{sweep_id}" / run_id
    else:
        checkpoint_dir = proj_dir / Path(cfg.callbacks.model_checkpoint.dirpath) / run_id
