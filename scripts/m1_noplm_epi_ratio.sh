...
# Based on best_playful_sweep_epi_ratio.sh with model.epiformer.use_plm=false
#
# Usage:
#   ./scripts/m1_noplm_epi_ratio.sh --gpu_id 0 --batch_size 8 --epochs 130 --server local
...
usage() {
    ...
    echo "Compare results with best_playful_sweep_epi_ratio.sh (with PLM) for M1 analysis."
    exit 1
}
...
run_id="m1_noplm_epi_ratio_seed${seed}_$(date +%Y%m%d-%H%M%S)"
